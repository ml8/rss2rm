...
func TestCredentialCRUD(t *testing.T) {
	...
	if cred.Name != "My Substack" {
		t.Fatalf("expected name 'My Substack', got %q", cred.Name)
	}
	if cred.Type != "substack_cookie" {
		t.Fatalf("expected type 'substack_cookie', got %q", cred.Type)
	}
	...
	if feed.CredentialID == nil || *feed.CredentialID != id {
		t.Fatalf("expected credential_id %q on feed, got %v", id, feed.CredentialID)
	}
